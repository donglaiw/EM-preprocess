...
//         default: throw "The filter Radius Tensor data type not supported.\n";    
//  }
...
at::Tensor cuda_median_3d(const at::Tensor& imStack, const at::Tensor& filtRads, int32_t halo)
{
    ...
    int32_t dimX = imStack.size(0), dimY = imStack.size(1), dimZ = imStack.size(2);
    auto f_copy = filtRads;
    auto fa = f_copy.accessor<float, 1>();
    int32_t radX = fa[0], radY = fa[1], radZ = fa[2];
    //Dispatch arguments for __median_3d
    dim3 blockDim(radX, radY, radZ);
    dim3 gridDim(

...
def read_h5(directory, dataset_name):
...
def test_with_cuda(ims, filter_shape):
    ...
    filter_torch = torch.tensor(filter_shape, device='cpu', dtype=torch.float32) / 2
...
class TestMedian(unittest.TestCase):
    ...
    def test_median(self):
        input_im_stack = read_h5(INPUT_FILE_DIR, DATASET_NAME)
        ...
        for i in range(len(out_cpu)):
            cv2.imwrite("original_%d.png" % (i + 1), input_im_stack[i])

...
def coarse2fine_flow(np.ndarray[double, ndim=3] Im1 not None,
    ...
    """
    h = Im1.size(0), w = Im1.size(1), c = Im1.size(2)
    vx = torch.zeros((h, w), dtype=torch.float32, device=Im1.device)
    vy = torch.zeros((h, w), dtype=torch.float32, device=Im1.device)
    wrapI2 = torch.zeros((h, w, c), dtype=torch.float32, device=Im1.device) \
        if wrap_step else torch.zeros((1, 1, 1), dtype=torch.float32,
                                      device=Im1.device)
